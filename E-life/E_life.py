...
def main():
    ...
    b_url = "https://elifesciences.org/archive/"
    years = [2024, 2025]

    archive_urls = []
    for year in years:
        full_url = b_url + str(year)
        archive_urls.append(full_url)
    for url in archive_urls:
        print(url)
        try:
            try:
                archive_soup = get_soup(url)
                listing_items = archive_soup.find("ol", class_="grid-listing").find_all("li", class_="grid-listing-item")
                link_list = []
            ...
            for current_link in link_list:
                ...
                try:
                    all_articles = current_soup.find("h3", string="Research articles").find_next_sibling("ol").findAll("li",class_="listing-list__item")
                    if all_articles:
